...
class NpyFeature(data.Dataset):
    ...
    def load_data(self, n_similar=3,batch_size = 16):
        ...
        # Load similar pairs
        rand_classid = np.random.choice(self.num_classes, size=n_similar)  # 选择3个不同类，分别为这三个类挑选pair
        for rid in rand_classid:
            class_name = list(self.action2video.keys())[rid]
            rand_sampleid = np.random.choice(len(self.action2video[class_name]), size=2)
        ...
        # Load rest pairs
        rand_sampleid = np.random.choice(len(self.vid_list), size=batch_size - 2 * n_similar)
...
class NpyFeature_UTAL(data.Dataset):
    ...
    def update_label(self, vid_name_list, label_list):
        assert len(vid_name_list) == len(label_list), 'the number of vid list is not equal label list !'
        self.anno = {}
    ...
    def load_data(self, n_similar=3,batch_size = 16):
        ...
        # Load similar pairs
        rand_classid = np.random.choice(len(self.cluster_action2video), size=n_similar)  # 选择3个不同类，分别为这三个类挑选pair
        for rid in rand_classid:
            class_name = list(self.cluster_action2video.keys())[rid]
            rand_sampleid = np.random.choice(len(self.cluster_action2video[class_name]), size=2)
...
class pairDataset(data.Dataset):
    def __init__(self, cfg, phase="train", sample="random"):
        ...
        self.feature_path = []
        for _modal in ['rgb', 'flow']:
            self.feature_path.append(os.path.join(self.data_dir, 'features', self.phase, _modal))
    ...
    def get_feature(self):
        ...
        for vid_name in self.data_list:
            ...
            con_vid_feature = np.concatenate((rgb_feature, flow_feature), axis=1)

            if self.sample == "random":
                con_vid_spd_feature = self.random_sample(con_vid_feature, self.sample_segments_num)
            else:
                con_vid_spd_feature = self.uniform_sample(con_vid_feature, self.sample_segments_num)
