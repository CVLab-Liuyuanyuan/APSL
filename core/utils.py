...
def get_proposal_dict_um(cas_pred, aness_pred, feat_magnitudes_np, pred, score_np, vid_num_seg, config):
    ...
    for th in config.MAGNITUDES_THRESH:
        ...
        seg_list = [np.where(feat_magnitudes_np_temp[:, c, 0] > 0) for c in range(len(pred))]
        proposals = get_proposal_oic(seg_list, cas_tmp, score_np, pred, config.UP_SCALE, \
                        vid_num_seg, config.FEATS_FPS, num_segments)
        for i in range(len(proposals)):
...
def set_path(config):
    if config.MODE == 'train':
        config.EXP_NAME = 'experiments/{cfg.MODE}/positive_{cfg.R_P}_negative_{cfg.R_N}' \
               '_m_{cfg.m}_M_{cfg.M}_freq_{cfg.TEST_FREQ}' \
               '_seed_{cfg.SEED} tr_utal3'.format(cfg=config)
